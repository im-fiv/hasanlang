...
fn test_update(command: TestUpdateCommand) {
	fn inner_test_update(case_name: String) {
		...
		let contents = fs::read_to_string(case_path)
			.expect("Failed to read test case file contents");

		// Parse into pest.rs pairs
		use pest::Parser;
		use hasan_pest_parser::{PestParser, Rule};

		let pest_parsed = PestParser::parse(
			Rule::program,
			&contents
		).ok().unwrap_or_else(|| panic!("Failed to pest-parse test case `{case_name}`"));

		// Parse into Hasan AST
		use hasan_parser::HasanParser;

		let hasan_parsed = HasanParser::new(pest_parsed)
			.parse();
		...
		fs::write(out_path, hasan_ast_str)
			.expect("Failed to write to the test case output file");
	}
	...
	if !all {
		let case_name = case_name
			.unwrap_or_else(|| unreachable!("The abscence of flag `--all` indicates the presence of `case_name`"));

		inner_test_update(case_name);
	} else {
		let paths = fs::read_dir("./tests/cases/")
